 ...
 */
package clases;
import conexionPHP.conexionphp;
...
public class entrada {
    ...
    public DefaultTableModel getDatosEntradas(){
        try {
            setTitulosEntrada();
            PS = CX.getConnection().prepareStatement(SQL_SELECT_ENTRADA);
            RS = PS.executeQuery();
    ...
    public int registrarEntrada(String nfactura, String codigo, Date fecha, int cantidad){
        int res=0;
        try {
            PS = CX.getConnection().prepareStatement(SQL_INSERT_ENTRADA);
